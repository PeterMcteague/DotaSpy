...
class MainWindow:
            ...
            if 100 >= self.MatchesToGet.get() > 0:
                ...
                if self.PlayerProfile.fetchSteamID() != "error":
                    ...
                    if self.PlayerProfile.CoreCount > self.PlayerProfile.SupportCount:
                        ...
                        self.RoleLabel = tk.Label(self.MainWindow, text = "In your last X matches, you've played support and core heroes equally."
                             , font = (self.MainFont,14), background = self.WindowBackgroundColor, fg="#ffffff").place(x=200, y=110)
                    ...
                    #Display
                    #Destroy previously displayed
                    try:
                        self.HeroPlayedLabel.destroy()
                    ...
                    if self.MatchesToGet.get() == 1:
                        tk.Label(self.MainWindow, background = self.WindowBackgroundColor, width=80, height=45).place(x=80, y=330)
                        tk.Label(self.MainWindow, background = self.WindowBackgroundColor, width=80, height=45).place(x=160, y=330)
                    elif self.MatchesToGet.get() == 2:  
                        tk.Label(self.MainWindow, background = self.WindowBackgroundColor, width=80, height=45).place(x=160, y=330)
    ...
    def __init__(self):
        ...
        tk.Label(self.MainWindow, text = "Scan last X games: "
                     , font = (self.MainFont,14), background = self.WindowBackgroundColor, fg="#ffffff").place(x=550, y=2)
        self.MatchesToGet = tk.IntVar()
        self.MatchesToGet.set(50)
        self.MatchesToGetEntry = tk.Entry(self.MainWindow, bd = 0, font = (self.MainFont,12), exportselection=0, textvariable = self.MatchesToGet, width=4).place(x = 700,y=5)
